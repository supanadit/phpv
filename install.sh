...
install_phpv() {
    ...
    if [ "$os" = "unknown" ] || [ "$arch" != "amd64" ] && [ "$arch" != "arm64" ]; then
        log_error "Unsupported platform: ${os}-${arch}. Currently only linux-amd64 and linux-arm64 are supported."
        exit 1
    fi
